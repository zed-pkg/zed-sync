...
class SimServer {
  final Map<String, ({Map<String, dynamic> row, Hlc version, int updatedAt})> _rows = {};
  ...
  ({Hlc version, ChangeEvent change}) commit(ChangeEvent c) {
    if (c.writeKey != null && _byKey.containsKey(c.writeKey)) {
      ...
      return (version: v, change: _feed(c.table, c.op, c.id, v, c.op == Op.delete ? null : c.row, c.writeKey));
    }
    final k = '${c.table} ${c.id}';
    final prev = _rows[k];
    final updated = _now() > (prev?.updatedAt ?? 0) ? _now() : (prev?.updatedAt ?? 0) + 1;
    Hlc version;
    ...
    if (c.op == Op.delete) {
      ...
      final wall = prev != null && prev.version.wallMs > updated ? prev.version.wallMs : updated;
      final counter = prev != null && prev.version.wallMs == wall ? prev.version.counter + 1 : 0;
    ...
    return (version: version, change: _feed(c.table, c.op, c.id, version, payload, c.writeKey));
  }

  ChangeEvent _feed(String table, Op op, String id, Hlc v, Map<String, dynamic>? row, String? key) =>
      ChangeEvent(table: table, op: op, id: id, version: v, atMs: v.wallMs, row: row, writeKey: key);

  Map<String, dynamic>? currentRow(String table, String id) => _rows['$table $id']?.row;
}
...
void main() {
  group('WriteMode', () {
    test('optimisticQueue applies locally, sends, and adopts the server HLC', () async {
      final s = SimServer();
      final c = connect(s, actor: 'dev-1');
      final res = await c.write('products', 'p1', {'id': 'p1', 'name': 'Ball'});
      expect(res.status, WriteStatus.acked);
      expect(res.version.actor, 'pg', reason: 'adopted the server-committed HLC');
      final row = await c.storage.getRow('products', 'p1');
      expect(row!.dirty, isFalse);
      expect((await c.storage.pending()).isEmpty, isTrue, reason: 'queue drained on ack');
    });

    test('localOnly applies + enqueues but never sends', () async {
      final s = SimServer();
      final sent = <ChangeEvent>[];
      final c = SyncClient(storage: MemoryStorage(), actor: 'dev-1', send: (ch) async {
        sent.add(ch);
        return s.commit(ch).version;
      });
      final res = await c.write('products', 'p1', {'id': 'p1'}, mode: WriteMode.localOnly);
    ...
    test('serverFirst commits before storing, and stores clean at the committed version', () async {
      final s = SimServer();
  ...
  group('ErrorPolicy on an offline send', () {
    Future<SyncClient> offline(ErrorPolicy p, {WriteMode mode = WriteMode.optimisticAwaitAck, List<String>? emitted}) async {
      return SyncClient(
        storage: MemoryStorage(),
        actor: 'dev-1',
        writeMode: mode,
    ...
    test('throwOnly rejects the write', () async {
      ...
      await expectLater(c.write('t', 'i', {'id': 'i'}), throwsA(isA<StateError>()));
      expect((await c.storage.pending()).length, 1, reason: 'still durably queued');
    });
    ...
    test('optimisticQueue swallows the send error and keeps it queued regardless of policy', () async {
      final c = await offline(ErrorPolicy.throwOnly, mode: WriteMode.optimisticQueue);
      final res = await c.write('t', 'i', {'id': 'i'}); // must NOT throw under optimisticQueue
      expect(res.status, WriteStatus.queued);
    });
  });
  ...
  group('conflict resolution', () {
    ...
    test('serverWins: a strictly-newer server change overrides a dirty local edit and drops its queued write', () async {
      final s = SimServer();
      final b = connect(s, actor: 'dev-B', conflictResolution: ConflictResolution.serverWins);
      // Clean baseline via the feed.
      await b.applyChange(ChangeEvent(table: 'docs', op: Op.upsert, id: 'd1', version: const Hlc(1000, 0, 'pg'), atMs: 1000, row: {'id': 'd1', 'title': 'orig'}));
      // B edits locally offline -> dirty, queued.
      await b.write('docs', 'd1', {'id': 'd1', 'title': 'B draft'}, mode: WriteMode.localOnly);
      ...
      final outcome = await b.applyChange(ChangeEvent(table: 'docs', op: Op.upsert, id: 'd1', version: incoming, atMs: incoming.wallMs, row: {'id': 'd1', 'title': 'A wins'}));
      expect(outcome, 'conflict-resolved');
      expect(domain(await b.storage.getRow('docs', 'd1')), {'id': 'd1', 'title': 'A wins'});
      expect((await b.storage.pending()).isEmpty, isTrue, reason: 'conflicting queued write dropped');
    });

    test('a newer local dirty edit survives a stale server echo (stale-ignore, not clobber)', () async {
      final s = SimServer();
      final a = connect(s, actor: 'dev-A');
      await a.write('docs', 'd1', {'id': 'd1', 'v': 1}); // acked, clean
      await a.write('docs', 'd1', {'id': 'd1', 'v': 2}, mode: WriteMode.localOnly); // dirty, newer
      ...
      final outcome = await a.applyChange(ChangeEvent(table: 'docs', op: Op.upsert, id: 'd1', version: stale, atMs: stale.wallMs, row: {'id': 'd1', 'v': 0}));
      expect(outcome, 'ignored');
      expect(domain(await a.storage.getRow('docs', 'd1')), {'id': 'd1', 'v': 2}, reason: 'newer local edit preserved');
      expect((await a.storage.pending()).length, 1, reason: 'the edit is still queued to send');
    });
  });

  group('offline + convergence', () {
    test('offline writes queue, then flushQueue drains them and a peer converges', () async {
      final s = SimServer();
      var up = false;
      ...
      expect(domain(await b.storage.getRow('notes', 'n1')), {'id': 'n1', 't': 'first'});
      expect(domain(await b.storage.getRow('notes', 'n2')), {'id': 'n2', 't': 'second'});
    });

    test('two devices converge on the same row and version; delete tombstone wins', () async {
      final s = SimServer();
      final b = connect(s, actor: 'dev-B');
      final a = connect(s, actor: 'dev-A', peers: () => [b]);

      await a.write('items', 'k1', {'id': 'k1', 'n': 1});
      expect(domain(await b.storage.getRow('items', 'k1')), {'id': 'k1', 'n': 1});
      final av = (await a.storage.getRow('items', 'k1'))!.version;
      final bv = (await b.storage.getRow('items', 'k1'))!.version;
      expect(av.compareTo(bv), 0, reason: 'both devices hold the identical committed HLC');

      await a.delete('items', 'k1');
      expect((await a.storage.getRow('items', 'k1'))!.row, isNull);
      expect((await b.storage.getRow('items', 'k1'))!.row, isNull, reason: 'delete tombstone propagated');
    });
  });

  group('timestamps', () {
    test('synced_at is stamped on adopt and preserved across a dirty edit', () async {
      final s = SimServer();
      final c = connect(s, actor: 'dev-1');
      ...
      expect(dirty.syncedAtMs, first, reason: 'a dirty edit preserves the prior synced_at');
    });
  });
}
