...
void main() {
  test('tick is monotonic even when the wall clock steps backwards', () {
    ...
    expect(c.tick(1500), _hlc(2000, 1, 'a'), reason: 'backwards clock -> counter bump, no regress');
    expect(c.tick(2000), _hlc(2000, 2, 'a'), reason: 'same ms -> counter bump');
    expect(c.tick(2001), _hlc(2001, 0, 'a'), reason: 'new ms -> counter resets');
  });
  ...
  test('observe folds an in-range remote and sorts strictly after it', () {
    ...
    expect(observed.compareTo(const Hlc(3000, 2, 'b')) > 0, isTrue, reason: 'local now sorts after remote');
  });

  test('observe folds a slightly-ahead in-range remote like the classic HLC update', () {
    final c = Clock('a');
  ...
  test('observe clamps a far-future (poison) remote wall to now + maxDriftMs', () {
    ...
    expect(observed.wallMs <= now + maxDriftMs, isTrue, reason: 'never advances past the drift bound');
    expect(observed.wallMs, now, reason: 'advanced to now, not the poisoned wall');
  ...
  test('actor breaks an equal (wall, counter) tie deterministically', () {
    expect(const Hlc(5, 5, 'aaa').compareTo(const Hlc(5, 5, 'bbb')) < 0, isTrue);
    expect(const Hlc(5, 5, 'bbb').compareTo(const Hlc(5, 5, 'aaa')) > 0, isTrue);
