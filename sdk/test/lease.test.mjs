...
test("startSync drains the inherited queue with fencing headers after winning the lease", async (t) => {
  ...
  const client = makeClient({ lock: () => makeHandle({ fencingToken: 41 }) });
  ...
  assert.equal(headers["idempotency-key"], "device-1-prev-0");
  assert.equal(headers["x-zed-sync-lease-key"], "zed-sync/flusher/zed-sync/device-1");
  assert.equal(headers["x-zed-sync-fencing-token"], "41");
...
test("startSync fails closed and sends nothing once the lease is lost", async (t) => {
  ...
  globalThis.fetch = async (url, init) => {
    fetchCalls.push({ url: String(url), init });
    return new Response(
      JSON.stringify({ committed_version: { wall_ms: 9, counter: 0, actor: "server" } }),
      { status: 200, headers: { "content-type": "application/json" } },
    );
  };
  globalThis.WebSocket = class {
  ...
  const { client: sync, lease, lifecycle, stop } = await startSync({
    ...
    store: new MemoryStore(),
    backend: { baseUrl: "http://backend.test" },
    lease: { client, ttlMs: 100, renewIntervalMs: 20, flushOnAcquire: false },
  });

  await sync.write("notes", "n1", { title: "while leader" });
  assert.equal(fetchCalls.at(-1).init.headers["x-zed-sync-fencing-token"], "7");
