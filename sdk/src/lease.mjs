...
// startSync threads it into every backend send as x-zed-sync-fencing-token /
// x-zed-sync-lease-key headers; the service must assert it inside the write
// transaction (`SELECT zed_sync.assert_fence(key, token, holder)` — see
// postgres/zed_sync.sql) so a paused ex-holder's writes are rejected. The
// queued-write Idempotency-Key remains the dedup backstop.
